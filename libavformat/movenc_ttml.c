    ...
    while (!avpriv_packet_list_get(&track->squashed_packet_queue, pkt)) {
        ...
        if (time_limited) {
            ...
            if (pkt->pts + pkt->duration > end_ts) {
                ...
                // put it back to list with adjusted timestamps
                ...
                track->squashed_packet_queue.head->pkt.pts =
                track->squashed_packet_queue.head->pkt.dts =
                track->squashed_packet_queue.head->pkt.pts + offset;
                track->squashed_packet_queue.head->pkt.duration -= offset;
