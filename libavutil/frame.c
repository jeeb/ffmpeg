...
static void wipe_side_data(AVFrameSideData ***sd, unsigned int *nb_side_data)
{
    for (unsigned int i = 0; i < *nb_side_data; i++) {
        free_side_data(&((*sd)[i]));
    }
    *nb_side_data = 0;
...
{
    av_assert0(frame->nb_side_data >= 0);
    wipe_side_data(&frame->side_data, &frame->nb_side_data);
}

void av_frame_side_data_free(AVFrameSideData ***sd, unsigned int *nb_sd)
{
    wipe_side_data(sd, nb_sd);
}

static void remove_side_data(AVFrameSideData ***sd, unsigned int *nb_side_data,
                             const enum AVFrameSideDataType type)
{
    for (unsigned int i = *nb_side_data - 1; i < UINT_MAX; i--) {
...
const AVFrameSideData *av_frame_side_data_get(const AVFrameSideData **sd,
                                              const unsigned int nb_sd,
                                              enum AVFrameSideDataType type)
{
    for (unsigned int i = 0; i < nb_sd; i++) {
        if (sd[i]->type == type)
            return sd[i];
    }
    return NULL;
