...
int ff_hevc_output_frame(HEVCContext *s, AVFrame *out, int flush)
{
    ...
    do {
        ...
        if (nb_output) {
            ...
            for (j = 0; j < 3; j++) {
                int off = ((frame->window.left_offset >> s->sps->hshift[j]) << s->sps->pixel_shift) +
                          (frame->window.top_offset >> s->sps->vshift[j]) * dst->linesize[j];
                dst->data[j] += off;
            }
...
int ff_hevc_slice_rpl(HEVCContext *s)
{
    ...
    for (list_idx = 0; list_idx < nb_list; list_idx++) {
        ...
        if (sh->collocated_list == list_idx &&
            ...
            rpl->ref[sh->collocated_ref_idx]->refPicList)
...
void ff_hevc_frame_rps(HEVCContext *s)
{
    ...
    for (i = 0; i < short_rps->num_delta_pocs; i++) {
        ...
        add_candidate_ref(s, &rps[list], poc, HEVC_FRAME_FLAG_SHORT_REF);
    }
    ...
    for (i = 0; i < long_rps->nb_refs; i++) {
        ...
        add_candidate_ref(s, &rps[list], poc, HEVC_FRAME_FLAG_LONG_REF);
    }
