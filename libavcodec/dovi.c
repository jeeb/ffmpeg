...
int ff_dovi_rpu_parse(DOVIContext *s, const uint8_t *rpu, size_t rpu_size)
{
    ...
    if (av_dovi_profile(hdr) == 5 && use_nlq) {
    ...
    if (vdr_dm_metadata_present) {
        ...
        for (int i = 0; i < 9; i++)
            color->ycc_to_rgb_matrix[i] = av_make_q(get_sbits(gb, 16), 8192);
        for (int i = 0; i < 3; i++) {
            int denom = av_dovi_profile(hdr) == 4 ? (1 << 30) : (1 << 28);
        ...
        for (int i = 0; i < 9; i++)
            color->rgb_to_lms_matrix[i] = av_make_q(get_sbits(gb, 16), 16384);
